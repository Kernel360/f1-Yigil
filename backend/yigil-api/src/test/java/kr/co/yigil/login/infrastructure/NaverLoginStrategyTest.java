...
@SpringBootTest(classes = NaverLoginStrategy.class)
class NaverLoginStrategyTest {
    ...
    @BeforeEach
    void setUp() {
        naverLoginStrategy.setRestTemplate(restTemplate);
    }

    @DisplayName("토큰이 유효하고 멤버가 존재하면 로그인이 잘 되는지")
    @Test
    void whenTokenIsValid_andMemberExists_thenLoginSuccessful() {
        NaverTokenInfoResponse mockResponse = new NaverTokenInfoResponse();
        mockResponse.setResponse(new NaverTokenInfoResponse.Response("12345678"));

        when(restTemplate.exchange(
                anyString(),
                eq(GET),
                any(),
                eq(NaverTokenInfoResponse.class))
        ).thenReturn(ResponseEntity.ok(mockResponse));

        Long memberId= 1L;
        Member mockMember = new Member(memberId,"email@example.com", "12345678", "user", "image_url", SocialLoginType.NAVER);

        when(memberReader.findMemberByEmailAndSocialLoginType(
                 anyString(), eq(SocialLoginType.NAVER)
        ...
        Long response = naverLoginStrategy.processLogin(loginCommand, "mockAccessToken");
    ...
    void whenTokenIsInvalid_thenThrowException() {
        ...
        Throwable thrown = catchThrowable(
                () -> naverLoginStrategy.processLogin(loginCommand, "invalidAccessToken"));
    ...
    void whenMemberIsNew_thenRegisterNewMember() {
        ...
        Long memberId = 1L;
        Member mockMember = new Member(memberId, "email@example.com", "12345678", "user",
                "image_url", SocialLoginType.KAKAO);
        when(memberReader.findMemberByEmailAndSocialLoginType("email@example.com",
                SocialLoginType.NAVER)).thenReturn(Optional.empty());
